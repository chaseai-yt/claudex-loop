...
- Codex CLI installed and recent: `codex --version` (need ≥ 0.130 — older CLIs error on the CLI's current default model; the default itself changes over time, so don't assume a specific model name, and upgrade the CLI rather than pinning `-m` to work around it).
